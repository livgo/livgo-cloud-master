...
public final class FileEncodingUtil {
    private final static Logger logger = LoggerFactory.getLogger(FileEncodingUtil.class);
    ...
    public static void convert(File file, String fromCharsetName, String toCharsetName, FilenameFilter filter) {
        if (file.isDirectory()) {
            List<File> list = ValidUtil.isNotEmpty(filter) ? FileUtil.listFileFilter(file, filter) :
                    FileUtil.listFile(file);
            if (ValidUtil.isNotEmpty(list)) {
                for (File f : list) {
