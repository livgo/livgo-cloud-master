...
public final class CollectionUtil {
    ...
    public static <T> void handler(Collection<T> collection, ObjectHandler<T> handler) {
        if (collection == null || collection.size() == 0) {
            LOG.error("collection is empty or is null");
            return;
        }
        for (T t : collection) {
            handler.handler(t);
    ...
    public static <T, E> void process(Collection<T> collection, Collection<E> result, ObjectProcess<T, E> process) {
        ...
        if (result == null) {
            LOG.error("result collection is null");
            return;
        }
        for (T t : collection) {
    ...
    public static <T> List<T> intersection(List<T> list1, List<T> list2) {
        if (ValidUtil.isNotEmpty(list1, list2)) {
    ...
    public static <T> Set<T> intersection(Set<T> set1, Set<T> set2) {
        if (ValidUtil.isNotEmpty(set1, set2)) {
    ...
    public static <T> Queue<T> intersection(Queue<T> queue1, Queue<T> queue2) {
        if (ValidUtil.isNotEmpty(queue1, queue2)) {
    ...
    public static <K, V> Map<K, V> intersection(Map<K, V> map1, Map<K, V> map2) {
        ...
        if (ValidUtil.isNotEmpty(map1, map2)) {
     ...
     * @return 交集
     */
    public static <T> Queue<T> union(Queue<T> queue1, Queue<T> queue2) {
    ...
    public static <T> Queue<T> subtract(Queue<T> queue1, Queue<T> queue2) {
        ...
        if (ValidUtil.isNotEmpty(queue1)) {
            queue.addAll(queue1);
